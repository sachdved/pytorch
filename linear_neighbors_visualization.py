#!/usr/bin/env python3
"""
Script to visualize the 10 neighbors of torch.nn.Linear in the knowledge graph.
"""
...
def main():
    ...
    # If there are edges, create a visualization
    if G.number_of_edges() > 0:
        ...
        for u, v, data in H.edges(data=True):
            edge_type = data.get('link_type', 'other')
            color = 'blue' if edge_type == 'inheritance' else 'gray'
            width = 2 if edge_type == 'inheritance' else 1
            nx.draw_networkx_edges(H, pos, edgelist=[(u, v)], edge_color=color,
        ...
        nx.draw_networkx_nodes(H, pos, node_color=node_colors, alpha=0.8,
                              node_size=1000)
        ...
        plt.title('Neighbors of torch.nn.Linear', fontsize=16, fontweight='bold')
        plt.axis('off')
        plt.tight_layout()
        plt.show()
    else:
        print("No edges found in the knowledge graph, cannot create visualization")
